...
class Comment extends Base
{
    ...
    public function listTopCommentedNews($limit = 5, $days_old = 30)
    {
        ...
        $sql = "
        SELECT t1.news_id, title, date_created, count(t1.id) as comments_cnt, max(created) AS last_date, min(created) AS first_date
        FROM (
        ...
        WHERE datediff(current_date, comments.created) <= " . $days_old . ") as t1
        GROUP BY news_id
    ...
    public function listTopCommentingUsers($limit = 5)
    {
        $limit_clause = ($limit > 0) ? ' LIMIT ' . $limit : '';
        $sql = "
        SELECT user_id, name as user_name, count(id) as comments_cnt, sum(likes_cnt) as likes, sum(dislikes_cnt) as dislikes 
        FROM (
        SELECT user_id, c.id, likes_cnt, dislikes_cnt, name 
        FROM comments c LEFT JOIN users u ON c.user_id = u.id WHERE active != 0) t1
        GROUP BY user_id
